...
```{r loaddata, echo = FALSE, results = "hide"}
  # Tree
      ...
      data$n_eff <- with(data, (4*(N_C*N_T)) / (N_C+N_T))
...
# Publication bias. Based on funnel plot, it looks like pub bias. Correct effect
  model1.1_pub <- rma.mv(SMDH ~ sqrt(1/n_eff), V = data$v_SMDH, random = list(~1|study, ~1|variable, ~1|species, ~1|obs), test = "t", data = data)
...
```{r funnel, fig.height=4.5, fig.width=4.5}
