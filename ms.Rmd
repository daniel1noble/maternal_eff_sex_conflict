...
# Exclude 4 studies that fail test. 3 effects are not defined in any case
  ...
  # We have a clear relationship bw SD and Mean, especially so on log scale, but raw scale not so much. We should check assumptions of lnCVR analysis
    ...
    data_long_mean <- data_long %>% filter(grepl("X_", name))  %>% mutate(obs = 1:n())
      data_long_sd <- data_long %>% filter(grepl("sd_", name)) %>% mutate(obs = 1:n()) %>% select(obs, name, value)
       data_long_n <- data_long %>% filter(grepl("N_", name))  %>% mutate(obs = 1:n()) %>% select(obs, name, value)
       
    # Now, merge data sets together. 
      
       final_long1 <- left_join(data_long_mean, data_long_sd, by = "obs")
        final_long <- left_join(final_long1, data_long_n, by = "obs") %>% 
                      rename("sd" = value.y,
                             "mean" = value.x,
                             "n"  = value,
                             "trt" = name.x) %>% 
    ...
    # Now that we have it in long format, we can calculate lnSD and lnX and their associated SE's
        
        final_long <- escalc(sdi = value.y, ni = value,  
                             measure = "SDLN", data = final_long, 
                             append = TRUE, var.names=c("lnSD","v_lnSD"))
        
         final_long <- escalc(mi = value.x, sdi = value.y, ni = value,  
                              measure = "MNLN", data = final_long, 
                              append = TRUE, var.names=c("lnX","v_lnX"))
        
        
    # Fit a MLMA model with metafor
        model1_lnSD <- rma.mv(lnSD ~ 1 + name.x + lnX, V = v_lnSD, 
                       random = list(~1 + name.x|study, 
                                     ~1|obs, 
                                     ~1|species), 
                       dfs = "contain", test = "t", 
