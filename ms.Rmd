...
#####################
    ...
    model1_lnCVR_harm <- rma.mv(logCVR ~ Harm_type, V = v_logCVR, 
                       ...
                       dfs = "contain", test = "t", 
                       method = "REML", data = data_cvr)
    robust_model1_lnCVR <- robust(model1_lnCVR_harm, cluster = data_cvr$study, vcov = "CR2")

    lncvr_results <- mod_results(robust_model1_lnCVR, mod = "Harm_type", data = data_cvr, group = "species")$mod_table
...
harm_plotvar <- ggplot(data_cvr, aes(x = Harm_type, y = logCVR)) + 
              ...
              annotate('text', x = (seq(1, 3, 1)), y = 1.8, 
        	  	...
        	  	# 95 %CI: branches
        	    geom_errorbar(data = lncvr_results,  aes(x = moderator, y = estimate, ymin = lowerCL, ymax = upperCL), 
        	                  height = 0, show.legend = FALSE, size = 0.5, width = 0.05) +
              geom_point(data = lncvr_results, aes(x = moderator, y = estimate, colour= moderator), size = 5)+
              geom_segment(aes(x = 1, y = 4.8, xend = 2, yend = 4.8), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both")) + 
              geom_segment(aes(x = 1, y = 5.5, xend = 3, yend = 5.5), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both")) +
              #geom_segment(aes(x = 2, y = -1, xend = 3, yend = -1), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both"))
              annotate('text', x = 1 + ((2-1) / 2), y = 5, label = TeX(contrast1), size = 4) + 
