...
robust_model1_lnSD <- robust(model1_lnSD, cluster = final_long$study, vcov = "CR2")
 ...
 randef <- confint(model1_lnSD)
  ...
  table_fixed <- data.frame( Parameter = c("Intercept", "Treatment (Control)", "ln Mean", ""), 
                            Estimates = c(coefs_lnSD, ""),
                       `Upper 95% CI` = c(ci.ub_lnSD, ""),
                       `Lower 95% CI` = c(ci.lb_lnSD, ""), check.names = FALSE, row.names = NULL)
...
dat <- data.frame()
  ...
  table_rand <- data.frame(     Parameter = c("Study", "Residual", "Species", "tau^2", "rho"), 
                                Estimates = dat$est, 
                           `Upper 95% CI` = dat$lb,
                           `Lower 95% CI` = dat$up,check.names = FALSE, row.names = NULL)
```
...
flextable(rbind(table_fixed,table_rand)) %>% 
      add_header_row(values = "Fixed Effects", colwidths = 3) %>% 
