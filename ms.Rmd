...
harm_plotvar <- ggplot(data_cvr, aes(x = Harm_type, y = logCVR)) + 
              ...
              annotate('text', x = (seq(1, 3, 1)), y = 2, 
                       label= paste0("italic(n)==", lncvr_results$n), parse = TRUE, size = 3.5) +     
              annotate('text', x = (seq(1, 3, 1)), y = 1.8, 
                       label= paste0("italic(k)==", lncvr_results$nstudy), parse = TRUE, size = 3.5) +     
        	  	# 95 %CI: branches
              ...
              #geom_segment(aes(x = 2, y = -1, xend = 3, yend = -1), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both"))
              annotate('text', x = 1 + ((2-1) / 2), y = 1.1, label = TeX(contrast1v), size = 4) + 
              annotate('text', x = 1 + ((3-1) / 2), y = 1.4, label = TeX(contrast2v), size = 4) 
            
plot <- (harm_plot + harm_plotvar) + plot_annotation(tag_levels = "A", tag_suffix = ")")

ggsave(plot = plot, filename = "fig2.png", path = "./output/figs", height = 7.416667, width = 13.805556, device = "png", dpi = 600)
plot
```
...
p1_1 <-   ggplot(data, aes(x = log(X_T), y = log(sd_T))) + 
      ...
      geom_abline() + 
      labs(x = "log Mean", y = "log SD") + theme_bw()

p2_2 <-ggplot(data, aes(x = X_T, y = sd_T)) + 
      geom_point(size = 2, col = "black") + 
      geom_point(aes(x = X_C, y = sd_C), col = "grey", size = 2) + 
      geom_abline() + 
      labs(x = "Mean", y = "SD") + theme_bw()

(p1_1 | p2_2) + plot_annotation(tag_levels = "A", tag_suffix = ")")
```
