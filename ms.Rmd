...
###################
  ...
  model1.1_pub_full <- rma.mv(SMDH ~ sqrt(inv_n_eff) + SSD_lnRR + Harm_type, V = data$v_SMDH, 
   ...
   model1.1_pub_full_het <- rma.mv(SMDH ~ sqrt(inv_n_eff) + SSD_lnRR + Harm_type, V = data$v_SMDH, 
                              ...
                              str = "HCS", test = "t", data = data)
      AICc(model1.1_pub_full, model1.1_pub_full_het) # Basically the same results so simplify model.
...
```{r pretty_table, tab.cap = "Number of effect sizes, studies and species along with overall meta-analytic mean (average SMDH) and 95% confidence (CI) and prediction intervals (PI). Heterogeneity estimates and 95% CI's are also provided for study, trait, species and total heterogeneity (excluding sampling variance)"}
