...
#devtools::install_github("daniel1noble/metaAidR", force = TRUE)
  ...
  pacman::p_load(tidyverse, metafor, brms, orchaRd, ape, phytools, metaAidR, orchaRd, gt, kableExtra, flextable, patchwork, latex2exp, MuMIn, equatiomatic, magrittr, data.table, forestmangr)
eval(metafor:::.MuMIn)
```
...
###################
  ...
  # Female lifespan
    model1.10 <- rma.mv(SMDH ~ scale(F_lifespan), V = data$v_SMDH, 
                       random = list(~1|study, ~1|obs, ~1|species), dfs = "contain", test = "t", 
                       method = "REML", data = data)
    robust_model1.10 <- robust(model1.10, cluster = data$study, vcov = "CR2")
    
    model1.11 <- rma.mv(SMDH ~ scale(F_lifespan) + Harm_type + zSSD, V = data$v_SMDH, 
                       random = list(~1|study, ~1|obs, ~1|species), dfs = "contain", test = "t", 
                       method = "REML", data = data)
...
We used the standardised mean difference with a small sample correction (i.e., Hedges' *g*) as our measure of effect size comparing female fitness measures across experimental treatments. To correct for the possibility that population variances in the two treatments differ, we made use of a corrected version of Hedges' *g* that controls for heteroscedastic population variances (hereafter called SMDH; [@Bonnet2008; @Bonnett2009]). Effect sizes were calculated by subtracting the treatment group mean from the control group mean and dividing by the pooled standard deviation. As such, positive effect sizes indicate that female 'fitness/traits' in control groups was higher than female 'fitness/traits' in treatment groups. 

We also tested whether the variability in female fitness between the control and treatment group differed using the log Coefficient of Variation Ratio (lnCVR) that compared the coefficient of variation between control and treatment groups [@nakagawa2015meta]. Given lnCVR makes certain assumptions about mean-variance relationships, which are not always upheld [see @nakagawa2015meta], we also ran an 'arm-based' analysis using the log of the standard deviation (lnSD) in a multilevel meta-analytic model. See Supplemental Materials for full details. Both results provided similar conclusions about the change in variability between control and treatment groups so we only present lnCVR. Given lnCVR is a log ratio, we can only use ratio scale data to compute the effect size [@nakagawa2015meta]. In addition, data is assumed to be normally distributed; we tested this assumption using a modified Geary's test proposed by @laj2011. Only 6 effect sizes failed the test, and three of these were not ratio scale data. As such, we excluded these effect size data from our lnCVR (and lnSD) analysis. In all cases, the CV of the control group was subtracted from the CV of the treatment group such that positive effect sizes suggest greater variance in the control group and negative effect sizes greater variability in the treatment group (after controlling for the mean). 

#### Meta-analysis
We analysed effect size data using the *metafor* @Metafor (vers. `r utils::packageVersion("metafor")`) package in R @R (vers. `r paste0(R.Version()$major, ".", R.Version()$minor)`). To estimate the overall effect of male-harm on female fitness (for both mean and variance) we first fit multi-level meta-analytic models (intercept only) @nakagawa2012methodological. We plotted overall meta-analytic means using the orchard plot package @NakaOrchard (vers. `r utils::packageVersion("orchard")`). Overall, we collected between `r n_eff_study[1]` to `r n_eff_study[2]` effect sizes per study. To control for sources of non-independence, we included study and species-level random effects @Noble2017. We also fit a model that included a species-level random effect with a phylogenetic correlation matrix derived using the Open Tree of Life Database @rotl (vers. `r utils::packageVersion("rotl")`). We used Grafen's method @Grafen to calculate branch lengths using the R package *ape* @ape (vers. `r utils::packageVersion("ape")`). Overall, a model containing only a species-level random effect variance was better supported than a model that estimated a phylogenetic variance ($\Delta_{AIC_{c}}$ = `r dAICc`). As such, we did not include phylogeny in our models. 
...
Publication bias results when studies with lower statistical power that find opposite patterns to the predicted effects (i.e., male harm increases female fitness) go unpublished. This can result in effects being upwardly biased overall. Evidence for publication bias can be roughly detected by inspecting a funnel plot for funnel asymmetry. However, any asymmetry identified may simply be the result of high effect size heterogeneity. As such, to more formally test for publication bias we used a new method that relies on fitting a MLMR model accounting for all the moderators available to explain variation in effects (i.e., all random and fixed effects). Using this model, we also include effect size sampling variance as a moderator @NakagawaPubBias. This approach has the benefit of being a formal way to both statistically test for publication bias (while accounting for as much effect size heterogeneity as possible) and correcting for it; providing a sensitivity analysis on how the effect is expected to change if we were to hypothetically observe the missing effects @NakagawaPubBias. It is still important to note that this corrected overall mean should be interpreted with caution. We can never know exactly how many studies, if any, are missing. As such, these should be viewed as a sensitivity analysis. 


```{r, variationanalysis}

###################
      # Variance in female fitness in harm and control treatment. Use lnCVR and lnVR (Nakagawa et al. 2015). On request of reviewer
####################
   ## First, have a look at mean-variance relationships. Pretty strong relationship in both Treatment and Control group, as expected. 

## Geary's test to check normality assumptions
data <- data %>% 
         mutate(geary_control = geary(X_C, sd_C, N_C),
                    geary_trt = geary(X_T, sd_T, N_T),
                   geary_test = ifelse(geary_control >= 3 & geary_trt >= 3, "pass", "fail"))
# How many fail?
    geary_res <- data %>% group_by(geary_test) %>% summarise(n = n()) %>%  data.frame()

# Exclude 4 studies that fail test. 3 effects are not defined in any case
    data_cvr <- data %>% filter(geary_test == "pass")

########################
    # Overall MLMA model
#########################    
## Overall intercept only models. Note that, for lnCVR, 3 effect sizes are based on non-ratio data so are not defined and are excluded.
    model1_lnCVR <- rma.mv(logCVR ~ 1, V = v_logCVR, 
                       random = list(~1|study, 
                                     ~1|obs, 
                                     ~1|species), 
                       dfs = "contain", test = "t", 
                       method = "REML", data = data_cvr)
    robust_model1_lnCVR <- robust(model1_lnCVR, cluster = data_cvr$study, vcov = "CR2")
    
    model1_lnVR <- rma.mv(logVR ~ 1, V = v_logVR, 
                       random = list(~1|study, 
                                     ~1|obs, 
                                     ~1|species), 
                       dfs = "contain", test = "t", 
                       method = "REML", data = data_cvr)
    robust_model1_lnVR <- robust(model1_lnVR, cluster = data_cvr$study, vcov = "CR2")
    
########################
    # Arm-based model
#########################    
     # We have a clear relationship bw SD and Mean, especially so on log scale, but raw scale not so much. We should check assumptions of lnCVR analysis
    
    # First, we need to reorganise the data in long format
    data_long <- data %>% filter(geary_test == "pass") %>%  
                pivot_longer(cols = c(14:19), values_to = "value", names_to = "name") %>% 
                data.frame()
    
    data_long_mean <- data_long %>% filter(grepl("X_", name))  %>% 
                      mutate(obs2 = 1:n())
      data_long_sd <- data_long %>% filter(grepl("sd_", name)) %>% 
                      mutate(obs2 = 1:n()) %>% 
                      select(obs2, name, value)
       data_long_n <- data_long %>% filter(grepl("N_", name))  %>% 
                      mutate(obs2 = 1:n()) %>% 
                      select(obs2, name, value)
       
    # Now, merge data sets together. Note that, we want to do some renaming and we need to remember that lnCVR is subtracting the CONTROL - TREATMENT, but we need to flip this around in lnSD model because it's TREATMENT - CONTROL. 
      
       final_long1 <- left_join(data_long_mean, data_long_sd, by = "obs2")
        final_long <- left_join(final_long1, data_long_n, by = "obs2") %>% 
                      rename("sd" = value.y,
                             "mean" = value.x,
                             "n"  = value,
                             "trt" = name.x) %>% 
                      mutate(trt = relevel(as.factor(trt), ref = "X_T"),
                             study.obs = paste0(obs, "_", study))
    
    # Now that we have it in long format, we can calculate lnSD and lnX and associated SE's for lnSD which can be accounted for in meta-analysis
        
        final_long <- escalc(sdi = sd, ni = n,  
                             measure = "SDLN", data = final_long, 
                             append = TRUE, var.names=c("lnSD","v_lnSD"))
        
         final_long <- escalc(mi = mean, sdi = sd, ni = n,  
                              measure = "MNLN", data = final_long, 
                              append = TRUE, var.names=c("lnX","v_lnX"))
        # write long_data for checking
         write.csv(final_long, file = "./data/meta_dat_long.csv", row.names = FALSE)
         
    # Fit a MLMA model with metafor
        model1_lnSD <- rma.mv(lnSD ~ 1 + trt + lnX, V = v_lnSD, 
                       random = list(~1 + trt|study.obs, 
                                     ~1 | study,
                                     ~1|obs2, 
                                     ~1|species), 
                       dfs = "contain", test = "t", 
                       method = "REML", data = final_long)
robust_model1_lnSD <- robust(model1_lnSD, cluster = final_long$study, vcov = "CR2")

#####################
# Harm Type Analysis
#####################

    model1_lnCVR_harm <- rma.mv(logCVR ~ Harm_type, V = v_logCVR, 
                       random = list(~1|study, 
                                     ~1|obs, 
                                     ~1|species), 
                       dfs = "contain", test = "t", 
                       method = "REML", data = data_cvr)
    robust_model1_lnCVR_harm <- robust(model1_lnCVR_harm, cluster = data_cvr$study, vcov = "CR2")

    lncvr_results <- mod_results(robust_model1_lnCVR_harm, mod = "Harm_type", data = data_cvr, group = "species")$mod_table
  
  # Add N
  n <- data_cvr %>% group_by(Harm_type) %>% summarise(n = n(), nstdy = length(unique(study)))
  lncvr_results$n <- n$n
  lncvr_results$nstudy <- n$nstdy
  names(lncvr_results)[1] <- "moderator"
  lncvr_results$loc <- 1:dim(lncvr_results)[1]

###############
  # Final tables
################

 coefs_lnSD <- coef(model1_lnSD)
 ci.lb_lnSD <- model1_lnSD$ci.lb
 ci.ub_lnSD <- model1_lnSD$ci.ub
 randef <- confint(model1_lnSD)
 
  table_fixed <- forestmangr::round_df(data.frame( Parameter = c("Intercept", "Treatment (Control)", "ln Mean", ""), 
                            Estimates = as.numeric(c(coefs_lnSD, "")),
                       `Upper 95% CI` = as.numeric(c(ci.ub_lnSD, "")),
                       `Lower 95% CI` = as.numeric(c(ci.lb_lnSD, "")), check.names = FALSE, row.names = NULL), digits = 3)
  
dat <- data.frame()
  
  for(i in 1:(length(randef)-1)){
    tmp <- randef[[i]][1]$random[1,]
    dat <- rbind(dat, tmp)
  }
  names(dat) <- c("est", "lb", "up")
  
  table_rand <- forestmangr::round_df(data.frame(     Parameter = c("Study", "Residual", "Species", "tau^2", "rho"), 
                                Estimates = dat$est, 
                           `Upper 95% CI` = dat$lb,
                           `Lower 95% CI` = dat$up,check.names = FALSE, row.names = NULL), digits = 3)
```
...
```{r funnel, fig.height=7.62500, fig.width=14.90278, fig.cap="A) Funnel plot of effect size as a function of precision (i.e., inverse of sampling standard error). B) orchaRd plot of the overall meta-analytic mean SMDH. C) orchard plot of overall meta-analytic mean for lnCVR. In B & C the mean is presented with 95% confidence intervals (thick black bars) and 95% prediction intervals (whiskers)."}
...
p3 <-  orchard_plot(model1_lnCVR, group = "species", data = data_cvr, 
...
p4 <- (p1 + (p2 / p3)) + plot_annotation(tag_levels = "A", tag_suffix = ")")
p4
ggsave(plot = p4, filename = "fig1.png", width = 12, height= 5, path = "./output/figs", units = "in", device="png", dpi = 600)
ggsave(plot = p4, filename = "fig1.pdf", path = "./output/figs", height = 5, width = 12, device = "pdf")
...
contrast1v <- paste0("$\\beta_{contrast}$ = ", round(robust_model1_lnCVR_harm$beta[2], digit = 2), ", 95% CI:", round(robust_model1_lnCVR_harm$ci.lb[2], digit = 2), " to ", round(robust_model1_lnCVR_harm$ci.ub[2], digit = 2), ", $p =", p_value(robust_model1_lnCVR_harm$pval[2]), "$")

contrast2v <- paste0("$\\beta_{contrast}$ = ", round(robust_model1_lnCVR_harm$beta[3], digit = 2), ", 95% CI:", round(robust_model1.5$ci.lb[3], digit = 2), " to ", round(robust_model1_lnCVR_harm$ci.ub[3], digit = 2), ", p = ", p_value(robust_model1_lnCVR_harm$pval[3]))
...
axis_size <- theme(axis.title = element_text(size = 20), 
...
harm_plot <- ggplot(data, aes(x = Harm_type, y = SMDH)) + 
              ...
              annotate('text', x = (seq(1, 3, 1)), y = 6.45, 
                       label= paste0("italic(n)==", robust_model1.5_2_PI$n), parse = TRUE, size = 5) +     
              annotate('text', x = (seq(1, 3, 1)), y = 6.15, 
                       label= paste0("italic(k)==", robust_model1.5_2_PI$nstudy), parse = TRUE, size = 5) +     
        	  	# 95 %CI: branches
              ...
              annotate('text', x = 1 + ((2-1) / 2), y = 5, label = TeX(contrast1), size = 5) + 
              annotate('text', x = 1 + ((3-1) / 2), y = 5.7, label = TeX(contrast2), size = 5) +
              axis_size
              #annotate('text', x = 2 + ((2-1) / 2), y = -1, label = TeX(contrast3), size = 2.5)

lncvr_results$moderator <- c("both", "direct", "indirect")

harm_plotvar <- ggplot(data_cvr, aes(x = Harm_type, y = logCVR)) + 
              geom_violin(alpha = 0.2, aes(fill = Harm_type, colour = Harm_type)) + 
              #geom_point() + don't need this because we have geom_jitter. It will otherwise double plot points
              ylim(c(-2, 2.5)) + 
              geom_jitter(aes(x = Harm_type, y = logCVR, fill = Harm_type, colour = Harm_type, size = 1/sqrt(v_logCVR)),
                          width = 0.08, alpha = 0.2) + 
              theme_classic() + 
		          theme(legend.background = element_blank(),
			                     legend.key = element_blank()) + 
              labs(x = "Harm Type", 
                   y = "log Coefficient of Variation (lnCVR)", 
                   size = TeX("Precision $\\left(\\frac{1}{\\sqrt{v_{lnCVR}}}\\right)$")) +
              scale_x_discrete(labels = c("Both", "Direct", "Indirect")) +
              guides(size = guide_legend(override.aes = list(linetype = 0))) + 
              scale_colour_discrete(guide = FALSE) +
              scale_fill_discrete(guide = FALSE) + 
              annotate('text', x = (seq(1, 3, 1)), y = 2, 
                       label= paste0("italic(n)==", lncvr_results$n), parse = TRUE, size = 5) +     
              annotate('text', x = (seq(1, 3, 1)), y = 1.8, 
                       label= paste0("italic(k)==", lncvr_results$nstudy), parse = TRUE, size = 5) +     
        	  	# 95 %CI: branches
        	    geom_errorbar(data = lncvr_results,  aes(x = loc, y = estimate, ymin = lowerCL, ymax = upperCL), 
        	                  height = 0, show.legend = FALSE, size = 0.5, width = 0.05) +
              geom_point(data = lncvr_results, aes(x = loc, y = estimate, colour= moderator), size = 5) +
              geom_segment(aes(x = 1, y = 1, xend = 2, yend = 1), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both")) + 
              geom_segment(aes(x = 1, y = 1.3, xend = 3, yend = 1.3), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both")) +
              #geom_segment(aes(x = 2, y = -1, xend = 3, yend = -1), arrow = arrow(length = unit(0.1, "cm"), angle = 90, end = "both"))
              annotate('text', x = 1 + ((2-1) / 2), y = 1.1, label = TeX(contrast1v), size = 5) + 
              annotate('text', x = 1 + ((3-1) / 2), y = 1.4, label = TeX(contrast2v), size = 5) +
              axis_size
            
plot <- (harm_plot + harm_plotvar) + plot_annotation(tag_levels = "A", tag_suffix = ")") & theme(plot.tag = element_text(size = 24))

ggsave(plot = plot, filename = "fig2.png", path = "./output/figs", height = 7.4, width = 20.8, device = "png", dpi = 600)
ggsave(plot = plot, filename = "fig2.pdf", path = "./output/figs", height = 7.4, width = 20.8, device = "pdf")
plot
```
...
predDat <- data.frame(predict(model1.5_nonZ , newmods = cbind(0, 0, seq(from = min(data$SSD_lnRR, na.rm = T), to = max(data$SSD_lnRR, na.rm = T), length.out = 100)), addx = TRUE))

p3 = ggplot(data, aes(x = SSD_lnRR, y = SMDH)) + 
    geom_point(aes(size = 1/sqrt(v_SMDH), colour = Harm_type), alpha = 0.6) + 
      theme_classic() + 
  ...
  geom_smooth(data = predDat, aes(x = X.SSD_lnRR , y = ci.ub), method =  "lm", formula = y~x,se = FALSE,lty = "dashed", color = "black") +
  geom_smooth(data = predDat, aes(x = X.SSD_lnRR , y = ci.lb), method =  "lm", formula = y~x,se = FALSE, lty ="dashed", color = "black") +
  geom_smooth(data = predDat, aes(x = X.SSD_lnRR , y = pred), method =  "lm", formula = y~x, se = FALSE, color = "black")
  #geom_abline(intercept = robust_model1.3_nonZ$beta[1], slope = robust_model1.3_nonZ$beta[2]) + axis_size# Note using model1.3 for plotting because this is marginalised across Harm_type. 

ggsave(plot = p3, filename = "fig3.png", path = "./output/figs", height = 7, width = 10, device = "png", dpi = 600)
ggsave(plot = p3, filename = "fig3.pdf", path = "./output/figs", height = 7, width = 10, device = "pdf")
p3
```
...
sperm_plot = ggplot(sperm_data, aes(x = SCR.SCI, y = SMDH)) + 
              ...
              annotate('text', x = (seq(1, 2, 1)), y = 6, 
                       label= paste0("italic(n)==", robust_model1.7_PI$n[1:2]), parse = TRUE, size = 5) +     
              annotate('text', x = (seq(1, 2, 1)), y = 5.7, 
                       label= paste0("italic(k)==", robust_model1.7_PI$nstdy[1:2]), parse = TRUE, size = 5) +     
        	  	# 95 %CI: branches
  ...
  annotate('text', x = 1.5, y = 5, label = TeX(contrast_sperm), size = 5) + 
    annotate('text', x = 1.5, y = 4.6, label = TeX(contrast_sperm2), size = 5) + 
    annotate('text', x = 1.5, y = 4.2, label = TeX(contrast_sperm3), size = 5) + axis_size

ggsave(plot = sperm_plot, filename = "fig4.png", path = "./output/figs", height = 7, width = 10, device = "png", dpi = 600)
ggsave(plot = sperm_plot, filename = "fig4.pdf", path = "./output/figs", height = 7, width = 10, device = "pdf")
sperm_plot
...
# Supplemental Analysis using lnSD

A number of effect size measures have been proposed to compare changes in variability between two groups. @nakagawa2015meta introduced the log Coefficient of Variation Ratio (lnCVR) and log Variance ratio (lnVR) as two key measures. lnVR makes no assumptions about mean-variance relationships, however, lnCVR assumes the relationship is linear on the natural scale [@nakagawa2015meta; @Senior2017; @Senior2016]. It was clear from our data that there was a strong mean-variance relationship (Fig. S\@ref(fig:figs1)), as such, we resorted to using lnCVR, particularly because treatments also had a strong effect on the mean (see main analysis). 

The lnCVR effect size does have limitations which are not always upheld in practice [@nakagawa2015meta]. As such, we tested whether these assumptions impacted our inferences by re-running analyses using the log Standard Deviation (lnSD) as our effect size while controlling for log Mean (lnX) using a multilevel meta-regression model using *metafor* (version `r utils::packageVersion("metafor")`) that included lnX along with a two-level categorical predictor for treatment (Control vs Treatment) as fixed effects to both control for mean-variance relationship and estimate the effect contrast between control and treatment groups. The effect contrast should approximate lnCVR if assumptions are upheld. As random effects, we included study, species, and a residual variance (given *metafor* does not include this by default). We also included a random slope for each control and treatment comparison within a study to model variation in effects across studies.

Overall, our results were largely consistent with our analysis using lnCVR, with average variability differences being estimated to be approximately similar (See Table S\@ref(tab:tables1)).

```{r, figs1, fig.align='center', fig.cap= "Mean-variance relationships between the standard deviation (SD) and mean in the control ('grey' points) and treatment groups ('black' points). A) Mean-variance relationship on the log scale and B) natural scale."}
p1_1 <-   ggplot(data, aes(x = log(X_T), y = log(sd_T))) + 
      geom_point(size = 2, col = "black") + 
      geom_point(aes(x = log(X_C), y = log(sd_C)), col = "grey", size = 2) + 
      geom_abline() + 
      labs(x = "log Mean", y = "log SD") + theme_bw() + axis_size

p2_2 <-ggplot(data, aes(x = X_T, y = sd_T)) + 
      geom_point(size = 2, col = "black") + 
      geom_point(aes(x = X_C, y = sd_C), col = "grey", size = 2) + 
      geom_abline() + 
      labs(x = "Mean", y = "SD") + theme_bw() +  axis_size

supp_plot <- (p1_1 | p2_2) + plot_annotation(tag_levels = "A", tag_suffix = ")") & theme(plot.tag = element_text(size = 24))
ggsave(plot = supp_plot , filename = "figS1.png", path = "./output/figs", height = 7, width = 10, device = "png", dpi = 600)
ggsave(plot = supp_plot, filename = "figS1.pdf", path = "./output/figs", height = 7, width = 10, device = "pdf")
```

```{r, tables1,  tab.cap = "Multi-level meta-regression model with log standard deviation (lnSD)"}
flextable(rbind(table_fixed,table_rand)) %>% 
      add_header_row(values = "Fixed Effects", colwidths = 4) %>% 
      bold(part = "header", i = 1) %>% 
      width(j = c(2,3,4), width = c(5, 2, 2)) %>%
      align(align = "center", part = "all") %>% 
      compose(i = 4, j = 3, value = as_paragraph(as_b("Random Effects"))) %>% 
