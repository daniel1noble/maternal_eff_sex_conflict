...
```{r loadpackages, echo = FALSE, results = "hide"}
  pacman::p_load(tidyverse, metafor, brms, metaAidR, ape, phytools, metaAidR, orchaRd, gt, kableExtra, flextable, patchwork, latex2exp, ggarrange)
  source("./R/func.R")
```
...
# Table 1: Effect, confidemce intervals, prediction intervals, heterogenity
      ...
      hetero_table <- I2(model1.1, v = data$v_SMDH) # Same as orchaRd::i2_ml(model1.1)
...
###################
  ...
  model1.3 <- rma.mv(SMDH ~ scale(SSD_lnRR), V = data$v_SMDH, random = list(~1|study, ~1|variable, ~1|species, ~1|obs), test = "t", data = data)
  ...
  model1.5 <- rma.mv(SMDH ~ Harm_type + scale(SSD_lnRR), V = data$v_SMDH, random = list(~1|study, ~1|obs, ~1|species), R = list(species = phylogeny), test = "t", data = data)
