...
# How many fail?
    ...
    model1_lnCVR <- rma.mv(logCVR ~ 1, V = v_logCVR, 
                       random = list(~1|study, ~1|obs, ~1|species), dfs = "contain", test = "t", 
                       method = "REML", data = data)
    robust_model1_lnCVR <- robust(model1_lnCVR, cluster = data$study, vcov = "CR2")
    
    model1_lnVR <- rma.mv(logVR ~ 1, V = v_logVR, 
                       random = list(~1|study, ~1|obs, ~1|species), dfs = "contain", test = "t", 
                       method = "REML", data = data)
    robust_model1_lnVR <- robust(model1_lnVR, cluster = data$study, vcov = "CR2")
    
  
    orchard_plot(model1_lnCVR, group = "study", data = data, 
                 xlab = "log Coefficient of Variation (lnCVR)") + 
