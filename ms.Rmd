...
We also tested whether the variability in female fitness between the control and treatment group differed. To test this question, we used two effect size measures, the log Coefficient of Variation Ratio (lnCVR) that compared the coefficient of variation between control and treatment groups (contrast-based effect size), and the log standard deviation (lnSD), controlling for the log mean (lnX) using an arm-based approach [@nakagawa2015meta]. Both methods assume a mean-variance relationship, which was clearly evident in our data (see Supp Fig. SX). However, lnCVR assumes a linear relationship on the natural scale (SD-Mean), whereas lnSD fit using an arm-based approach assumes they are linear on the log scale, but relaxes assumptions about how the mean and variance scale.
...
robust_model1_lnSD <- robust(model1_lnSD, cluster = final_long$study, vcov = "CR2")
  ...
  data_cvr %>% ggplot(aes(y = 1/ sqrt(v_logCVR), x = mean(logCVR))) + geom_point()
