...
pred <- predict(robust_model1.5, newmods = cbind(1, 0, 0, scale(seq(from = min(data$SSD_lnRR, na.rm = T), to = max(data$SSD_lnRR, na.rm = T), length.out =100), scale=FALSE)))
...
p3 = ggplot(data, aes(x = SSD_lnRR, y = SMDH, size = 1/sqrt(v_SMDH), colour = Harm_type)) + 
  ...
  #geom_smooth(aes(x = SSD_lnRR, y = robust_model1.3_nonZ$ci.lb[1]), method =  "lm", formula = y~x,se = FALSE,lty = "dashed") +
  #geom_smooth(aes(x = SSD_lnRR, y = robust_model1.3_nonZ$ci.ub[1]), method =  "lm", formula = y~x,se = FALSE, lty ="dashed")
  #geom_smooth(aes(x = SSD_lnRR, y = robust_model1.3_nonZ$beta[1] + robust_model1.3_nonZ$beta[2]*SSD_lnRR), method =  "lm", formula = y~x, se = FALSE)
