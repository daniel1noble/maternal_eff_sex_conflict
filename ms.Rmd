...
robust_model1_lnSD <- robust(model1_lnSD, cluster = final_long$study, vcov = "CR2")
 ...
 randef <- confint(model1_lnSD)
  ...
  table_fixed <- data.frame( name = c("Intercept", "Treatment (Control)", "ln Mean"), 
                            Estimates = coefs_lnSD,
                       `Upper 95% CI` = ci.ub_lnSD,
                       `Lower 95% CI` = ci.lb_lnSD, check.names = FALSE, row.names = NULL)
...
dat <- data.frame()
  ...
  table_rand <- data.frame(     Name = c("Study", "Residual", "Species", "tau^2", "rho"), 
                                Estimate = dat$est, 
                           `Upper 95% CI` = dat$lb,
