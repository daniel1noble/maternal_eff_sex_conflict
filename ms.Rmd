...
###################
  ...
  # Sex differences in lifespan
    ...
    model1.9 <- rma.mv(SMDH ~ sex_life + zSSD + Harm_type, V = data$v_SMDH, 
                       random = list(~1|study, ~1|obs, ~1|species), test = "t", 
                       method = "REML", data = data)
    robust_model1.9 <- robust(model1.8, cluster = data$study, vcov = "CR2")
    
  # Female lifespan
    model1.10 <- rma.mv(SMDH ~ scale(F_LRS), V = data$v_SMDH, 
                       random = list(~1|study, ~1|obs, ~1|species), test = "t", 
                       method = "REML", data = data)
    robust_model1.10 <- robust(model1.9, cluster = data$study, vcov = "CR2")
    
    model1.11 <- rma.mv(SMDH ~ scale(F_LRS) + Harm_type, V = data$v_SMDH, 
                       random = list(~1|study, ~1|obs, ~1|species), test = "t", 
                       method = "REML", data = data)
...
ft <- flextable::set_caption(ft,  caption = "Effect size", autonum = autonum)
ft %>%
